...
export default function ProductCard({ product }) {
  ...
  // Harga per satuan terkecil (jika isi > 1)
  const hargaSatuan = isi > 1 ? Math.round(harga_retail / isi) : null
  const hargaGrosirSatuan = (hasGrosir && isi > 1) ? Math.round(harga_grosir / isi) : null

  const waText = encodeURIComponent(
    `Halo kak, saya mau tanya/pesan:\n*${nama_item}*\nKode: ${kode_item || '-'}\nHarga Retail: ${formatRp(harga_retail)}/${satuan}\n\nMohon info ketersediaan stok ya 🙏`
  )
  ...
  return (
    ...
    >
      ...
      <div style={{ height: 4, background: categoryColor(jenis) }} />
        ...
        <div style={{
          ...
          {hasGrosir && (
            <div>
              ...
              {hargaGrosirSatuan && (
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <span style={{ fontSize: 10, color: 'var(--blue)', fontWeight: 600, opacity: 0.7 }}>
                    ≈ {formatRp(hargaGrosirSatuan)}/pcs
                  </span>
                </div>
              )}
            </div>
